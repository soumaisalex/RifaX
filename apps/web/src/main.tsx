...
function CheckoutRoute({ slug }: { slug: string }) {
  ...
  useEffect(() => { fetch(`/api/public/raffles/${slug}`).then((response) => (response.ok ? response.json() : null)).then(setRaffle).finally(() => setLoading(false)); }, [slug]);
  if (loading) return <main className="center-state"><p>Carregando…</p></main>;
  if (!raffle) return <main className="center-state"><h1>Rifa não encontrada</h1></main>;
  const params = new URLSearchParams(window.location.search);
  const selectedNumbers = (params.get("numbers") ?? "").split(",").map(Number).filter(Number.isFinite);
