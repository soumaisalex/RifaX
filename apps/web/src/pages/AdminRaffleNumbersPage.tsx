...
export default function AdminRaffleNumbersPage({raffleId}:{raffleId:string}){
 ...
 useEffect(()=>{fetch(`/api/admin/raffles/${raffleId}/numbers?status=${filter}`,{headers:auth()}).then(r=>r.json()).then(d=>setNumbers(d.numbers??[]));},[raffleId,filter]);
 const loadOrders=()=>fetch(`/api/admin/orders?raffleId=${raffleId}&search=${encodeURIComponent(search)}`,{headers:auth()}).then(r=>r.json()).then(d=>setOrders(d.orders??[]));
