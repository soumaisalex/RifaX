...
type Order={orderId:string;status:string;total:string;reservationExpiresAt?:string|null;paymentStatus?:string;pix?:{payload?:string|null}|null;numbers:number[];raffleTitle:string};
...
export default function OrderConfirmationPage({token}:{token:string}){
 ...
 const payload=order?.pix?.payload??"";
 const copy=async()=>{if(!payload)return;await navigator.clipboard.writeText(payload);setCopied(true);setTimeout(()=>setCopied(false),1800)};
 if(!order)return <main className="order-page"><div className="skeleton"/></main>;
 const mm=String(Math.floor(seconds/60)).padStart(2,"0"),ss=String(seconds%60).padStart(2,"0");
 return <main className="order-page"><section className="order-card"><span className="section-label">RIFA X · PEDIDO</span><h1>{order.status==="PAID"?"Pagamento confirmado":order.status==="EXPIRED"?"Reserva expirada":"Seu pedido está reservado"}</h1><p>{order.raffleTitle}</p><div className="order-total"><span>Total</span><strong>{money(order.total)}</strong></div><div className="order-numbers">{order.numbers.map(n=><span key={n}>{String(n).padStart(3,"0")}</span>)}</div>{order.status==="PENDING"&&<><div className="countdown"><span>Reserva válida por</span><strong>{mm}:{ss}</strong></div>{payload&&<><div className="pix-qr"><PixQrCode payload={payload}/></div><button className="pix-copy" onClick={copy}>{copied?"Código copiado ✓":"Copiar Pix Copia e Cola"}</button></>}<small>Após pagar, o administrador confirmará o Pix. Esta página verifica automaticamente o status.</small></>}{order.status==="PAID"&&<div className="success">Pagamento confirmado. Seus números estão garantidos.</div>}{order.status==="EXPIRED"&&<div className="expired">A reserva expirou. Escolha novos números para continuar.</div>}<button onClick={()=>navigator.share?.({title:order.raffleTitle,url:location.href})}>Compartilhar pedido</button></section></main>;
}
