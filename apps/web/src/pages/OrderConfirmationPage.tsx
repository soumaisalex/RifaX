...
type Order={id:string;status:string;buyer?:{name:string;phone:string};total:string;reservationExpiresAt:string;payment?:{status:string;pixCopyPaste?:string;qrCode?:string};numbers?:number[];raffle?:{title:string}};
...
export default function OrderConfirmationPage({token}:{token:string}){
 ...
 useEffect(()=>{fetch(`/api/public/orders/${token}`).then(r=>r.ok?r.json():null).then(setOrder)},[token]);
