...
app.post("/:raffleId/draw", requireRole(["SUPER_ADMIN","ORGANIZATION_ADMIN"]), async c => {
  ...
  const auth=c.get("auth") as {userId:string;organizationId:string|null};
