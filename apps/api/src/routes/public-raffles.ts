...
publicRaffles.get("/:slug", async (c) => {
  ...
  const raffle = await db.query.raffles.findFirst({ where: and(eq(raffles.slug, c.req.param("slug")), eq(raffles.status, "ACTIVE"), isNull(raffles.deletedAt)) });
...
publicRaffles.get("/:slug/numbers", async (c) => {
  const db = createDatabase(c.env.DATABASE_URL);
  const raffle = await db.query.raffles.findFirst({ where: and(eq(raffles.slug, c.req.param("slug")), eq(raffles.status, "ACTIVE"), isNull(raffles.deletedAt)), columns: { id: true } });
  if (!raffle) return c.json({ error: "Raffle not found" }, 404);
