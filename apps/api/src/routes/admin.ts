...
admin.get("/raffles", async (c) => {
  ...
  const rows = await db.query.raffles.findMany({ where: isNull(raffles.deletedAt), columns: { id: true, slug: true, title: true, status: true, ticketPrice: true, numbersCount: true, drawAt: true, createdAt: true }, orderBy: (table, { desc }) => [desc(table.createdAt)] });
  return c.json({ raffles: rows });
});

admin.post("/raffles", async (c) => {
  if (!requireAdmin(c)) return c.json({ error: "Unauthorized" }, 401);
  const body = await c.req.json<{ organizationId?: string; title?: string; slug?: string; description?: string; ticketPrice?: string; numbersCount?: number; drawMethod?: "RIFA_X" | "FEDERAL_LOTTERY"; drawAt?: string; pixKey?: string; pixCity?: string; prizes?: { title: string; description?: string; imageUrl?: string }[] }>();
  if (!body.organizationId || !body.title?.trim() || !body.slug?.trim() || !body.ticketPrice || !body.numbersCount || body.numbersCount < 1) return c.json({ error: "Invalid raffle data" }, 400);
  ...
  try {
    const raffle = await db.transaction(async (tx) => {
      const [created] = await tx.insert(raffles).values({ organizationId, title, slug, description: body.description?.trim() || null, ticketPrice, numbersCount, drawMethod: body.drawMethod ?? "RIFA_X", drawAt: body.drawAt ? new Date(body.drawAt) : null, pixKey, pixCity, status: "DRAFT" }).returning();
