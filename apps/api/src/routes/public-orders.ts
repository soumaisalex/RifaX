...
publicOrders.post("/raffles/:raffleId/orders", async (c) => {
  ...
  const name = body.name?.trim(); const phone = body.phone?.trim(); const numbers = body.numbers ?? [];
  if (!name || name.length > 160 || !phone || !PHONE.test(phone) || !numbers.length || numbers.length > 1000) return c.json({ error: "Invalid checkout data" }, 400);
