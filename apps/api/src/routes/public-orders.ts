...
import { raffles, orders, orderItems, raffleNumbers, payments } from "@rifa-x/database/schema";
...
publicOrders.get("/orders/by-phone/:phone", async (c) => {
  ...
  const rows = await db.select({ id: orders.id, raffleId: orders.raffleId, raffleTitle: raffles.title, status: orders.status, total: orders.total, createdAt: orders.createdAt }).from(orders).innerJoin(raffles, eq(raffles.id, orders.raffleId)).where(eq(orders.buyerPhone, phone)).orderBy(desc(orders.createdAt)).limit(20);
