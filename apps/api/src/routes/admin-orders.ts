...
const scope = (session: any) => session.role === "SUPER_ADMIN" ? undefined : eq(raffles.organizationId, session.organizationId);
...
async function updatePayment(c: any, paymentStatus: "CONFIRMED" | "CANCELLED") {
  const db = createDatabase(c.env.DATABASE_URL); const session = c.get("auth");
  const [row] = await db.select({ order: orders, raffle: raffles, payment: payments }).from(orders).innerJoin(raffles, eq(raffles.id, orders.raffleId)).leftJoin(payments, eq(payments.orderId, orders.id)).where(and(eq(orders.id, c.req.param("id")), scope(session))).limit(1);
